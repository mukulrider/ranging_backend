...
#                     buyer_df = heirarchy_check[['buyer']].drop_duplicates()
    ...
    def get(self, request, format=None):
        ...
        if default is None:
            if not args:
                ...
                need_state_list = args_list.pop('need_state__in', None)
                if need_state_list is None:
                    need_state_list = None
                ...
                #     product_list = None
                com_list = [bc_list, buyer_list, jr_buyer_list, psg_list,
                            need_state_list,
                            brand_name_list]  # ,brand_name_list,product_list]
                #print(com_list)
                ...
                for i in heirarchy.columns:
                    ...
                    if 'admin' in designation:
                        if col_list2 is not None:
                            ...
                            # print(col_df)
                            heirarchy_check = read_frame(
                                nego_ads_drf.objects.filter(buying_controller__in=bc_list).values(
                                                                                                     'buying_controller',
                                                                                                     'buyer',
                                                                                                     'junior_buyer',
                                                                                                     'product_sub_group_description',
                            ...
                            # print(kwargs)
                            heirarchy_check = read_frame(
                                nego_ads_drf.objects.filter(**kwargs))
...
class product_impact_filters_new(APIView):
    def get(self, request, format=None):
        ...
        buyer_header = args.pop('buyer_header__in', None)
        # header over
        cols = ['buying_controller', 'buyer', 'junior_buyer', 'product_sub_group_description','brand_indicator',
                'brand_name','parent_supplier','long_description']
        if 'admin' in designation:
            kwargs_header = {}
        else:
            if buyer_header is None:
        ...
        if default is None:
            if not args:
                df = read_frame(product_hierarchy.objects.filter(**kwargs_header).filter(**args))
                heirarchy = read_frame(
                    product_hierarchy.objects.filter(**kwargs_header).values('buying_controller', 'buyer','junior_buyer',
                                                                             'product_sub_group_description', 'brand_indicator','brand_name',
                                                                             'parent_supplier','long_description','base_product_number'))


                for i in heirarchy.columns:
                    #print(i)
                    if i == "base_product_number":
                        break
                    data = {i: df[i].unique()}
                    #print(data)
                    col = pd.DataFrame(data)
                    ...
                    a['id'] = i
                    a['title'] = i
                    if i == ('buying_controller') or i ==('long_description'):
                        a['required'] = True
                    else:
                        a['required'] = False
                    a['items'] = col_df_final
                ...
                if 'admin' in designation:
                    heirarchy = read_frame(
                        product_hierarchy.objects.values('buying_controller', 'buyer','junior_buyer',
                                                                             'product_sub_group_description', 'brand_indicator','brand_name',
                                                                             'parent_supplier','long_description','base_product_number'))
                else:
                    heirarchy = read_frame(
                ...
                if brand_name_list is None:
                    brand_name_list = None
                long_description_list = args_list.pop('base_product_number__in', None)
                if long_description_list is None:
                    long_description_list = None
                com_list = [bc_list,buyer_list,jr_buyer_list,psg_list,
                            brand_indicator_list,brand_name_list,parent_supplier_list,long_description_list]
                #print(com_list)
                args_list2 = {reqobj + '__in': request.GET.getlist(reqobj) for reqobj in request.GET.keys()}
                #com_list = [bc_list, buyer_list, jr_buyer_list, psg_list, brand_indicator_list, parent_supplier_list,
                #            supplier_list, ]
                ...
                for i in heirarchy.columns:
                    ...
                    if i == "base_product_number":
                        break
                    if i == "long_description":
                        col_name = 'base_product_number__in'
                        col_list2 = args_list2.pop(col_name, None)
                    else:
                        col_name = i + '__in'
                        col_list2 = args_list2.pop(col_name, None)
                    ...
                    if 'admin' in designation:
                        if i == ('buying_controller'):
                            ...
                            if col_list2 is not None:
                                ...
                                # print(col_df)
                                heirarchy_check = read_frame(
                                    product_hierarchy.objects.filter(buying_controller__in=bc_list).values(
                                        'buying_controller', 'buyer','junior_buyer',
                                        'product_sub_group_description','brand_indicator','brand_name','parent_supplier',
                                        'long_description','base_product_number'))
                                # print("inside buyerrr..")
                                col['selected'] = True
                                col['highlighted'] = False
                                ...
                                # print(kwargs)
                                heirarchy_check = read_frame(
                                    product_hierarchy.objects.filter(**kwargs))
                                col_df_check = pd.merge(col_df_heirarchy,
                                                        heirarchy_check[[i]].drop_duplicates(), how='right')
                                #print("after merge_1...")
                                # print(col_df_check)
                        ...
                        if i == ('buying_controller'):
                            ...
                            if col_list2 is not None:
                                ...
                                col['selected'] = True
                                col['highlighted'] = False
                                kwargs = { 'buying_controller__in': bc_list,
                                      'buyer__in': buyer_list,
                                      'junior_buyer__in': jr_buyer_list,
                                      'brand_indicator__in': brand_indicator_list,
                                      'product_sub_group_description__in': psg_list,
                        ...
                        if i == ('buying_controller') or i ==('buyer'):
                            ...
                            print("main else parrtttt")
                            if col_list2 is not None:
                                print("-----")
                                print("else part")
                                print(col_df)
                                heirarchy_check = read_frame(
                                        product_hierarchy.objects.filter(buying_controller__in=bc_list).values('buying_controller', 'buyer','junior_buyer',
                                    'product_sub_group_description','brand_indicator','brand_name','parent_supplier',
                                    'long_description','base_product_number'))
                                # print("inside buyerrr..")
                                col['selected'] = True
